/*
각종 컴포넌트를 올려놓기 전에, 어떤 방식으로 올려놓을지를 결정하는 것을 배치라고 하며
AWT에서 지원하는 배치는 다음과 같이 5가지 유형이 있다.
1) FlowLayout: 행 방향으로 늘서는 배치방식, 만일 공간이 부족하면 다음 행으로 
					내려간다. 컨테이너의 크기에 따라 흐르듯 배치가 되므로 flow
					라는 뜻이 부여되었다.
2) BorderLayout: 북, 동, 서, 남, 중앙 이라는 방위를 갖는 배치방법
3) GridLayout: 격자 모양의  배치방법
4) CardLayout: 카드처럼 하나의 카드가 보일 때는 다른 카드가 안 보이는 방식의 배치
...
class BorderTest{
	public static void main(String[] args){
		...
		//버튼을 프레임에 붙이기
		//상수(public static final)은 대문자로 가는 것이 원칙
		...
		frame.add(bt_center);//가운데에 버튼 부착, 방향을 명시하지 않으면 center이다
